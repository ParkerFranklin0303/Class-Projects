...
//
void srcML::mainHeader(const std::vector<std::string>& profileName, const std::vector<std::string>& fileName) {
...
//
// START HERE! / Use print function as a base to implement Milestone 1.
//
...
void AST::swap(AST& rhs) {
    ...
    nodes tempNodeType;
    std::string tempTag, tempCloseTag;
    ...
    // Switch node type based on Category(tags) or Token/whitespace(text).
    switch (nodeType) {
    case category:
        tempTag = tag;
        tag = rhs.tag;
...
void AST::mainHeader(const std::vector<std::string>& profileName, const std::vector<std::string>& fileName) {
    AST* tempNode(getChild("function"));
    std::list<AST*>::iterator ptr = tempNode->child.begin();

    child.insert(ptr, new AST(token, "#include \"profile.hpp\""));
    for (size_t i = 0; i < profileName.size(); ++i) {
        child.insert(ptr, new AST(token, "\nprofile " + profileName[i] + "(\"" + fileName[i] + "\");"));
    }
    child.insert(ptr, new AST(whitespace, "\n\n"));
}
...
void AST::mainReport(const std::vector<std::string>& profileName) {
    // Finds "main" function node.
    std::list<AST*>::iterator ptr = child.begin();
    std::string funcName = "";
    while (funcName != "main") {
        while (((*ptr)->tag != "function") && (ptr != child.end())) {
            ++ptr;
        }
        AST* tempNode = (*ptr)->getChild("name");
        funcName = tempNode->getName();
        if (funcName != "main") {
            ++ptr;
        }
    }

    // Searches for "block" node.
    ptr = (*ptr)->child.begin();
    while (((*ptr)->tag != "block") && (ptr != child.end())) {
        ++ptr;
    }

    // Sets a pointer to the last child and searches for "return" tag backwards.
    std::list<AST*>::iterator end = (*ptr)->child.end();
    --end;
    while (((*end)->tag != "block_content") && (end != (*ptr)->child.begin())) {
        --end;
    }
    ptr = end;
    end = (*end)->child.begin();
    while (((*end)->tag != "return") && (end != (*ptr)->child.end())) {
        ++end;
    }

    // Inserts each report statement.
    for (size_t i = 0; i < profileName.size(); ++i) {
        child.insert(end, new AST(token, "std::cout << " + profileName[i] + " << std::endl;\n" + "    "));
    }
}
...
void AST::functionCount(const std::string& profileName) {
    std::list<AST*>::iterator ptrFunc;
    std::string funcName = "";
    for (std::list<AST*>::iterator ptr = child.begin(); ptr != child.end(); ++ptr) {
        // Checks to see if current child is a "function" node.
        if ((*ptr)->tag != "function") { continue; }
        else {
            // Gets the name of the function.
            AST* tempNode = (*ptr)->getChild("name");
            funcName = tempNode->getName();

            // Searches for the "block" node and places the function count statement.
            tempNode = (*ptr)->getChild("block");
            ptrFunc = tempNode->child.begin();
            while (((*ptrFunc)->tag != "block_content") && (ptrFunc != tempNode->child.end())) {
                ++ptrFunc;
            }
            child.insert(ptrFunc, new AST(token, " " + profileName + ".count(__LINE__, \"" + funcName + "\");"));
        }
    }
}
    ...
    for (std::list<AST*>::const_iterator i = child.begin(); i != child.end(); ++i) {
        if ((*i)->nodeType == category) {
            if ((*i)->tag == "expr_stmt") {
                ...
                if (!(isStopTag((*i)->tag))) { (*i)->lineCount(profileName); }
            }
        }
        else {
            continue ;
...
//
// Can use this code as a base for Copy Constructor/Destructor and search functions that don't print anything!
//
...
// REQUIRES: '>' was previous character read 
